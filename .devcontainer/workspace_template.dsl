/*
  Пример workspace.dsl, демонстрирующий правила из документации ПТР.
  См.: start.md, 3.1-3.8, 4.1-4.2, 5, 9.1, 10, Регистрация в репозитории.

  Требуемая структура проекта:
  - workspace.dsl (в корне)
  - docs/          (документация для !docs)
  - decisions/     (ADR в формате 0001-название.md для !adrs)
  - themes/        (beeline_basic.json, beeline_tr_icons.json)
*/
workspace "Пример системы" "Демонстрация всех правил workspace.dsl" {
    ...
    model {
        ...
        ext_idp = softwareSystem "IDP" {
            tags "landscape"
            # properties всегда должен быть многострочный (открывающая и закрывающая скобка на разных линиях)
            properties {
                cmdb "IDP"
            }
        }
    ...
    views {
        ...
        # Связи на dynamic lиаграмме могут существовать только при наличии связи в модели (например, example.api -> example.db )
        dynamic example "001" {
            autoLayout
